 ...
 */
class ForwardingService(args: Array<String>) : Closeable {
    private var network: BitcoinNetwork? = null
    private var forwardingAddress: Address? = null
    ...
    fun run() {
        ...
        println("Forwarding address: $forwardingAddress")

        // Create and start the WalletKit
        kit = WalletAppKit.launch(network, File("."), getPrefix(network), MAX_CONNECTIONS)

        // Add a listener that forwards received coins
        kit?.wallet()
            ?.addCoinsReceivedEventListener { wallet: Wallet, incomingTx: Transaction, prevBalance: Coin, newBalance: Coin ->
                coinForwardingListener(
                    wallet,
                    incomingTx,
                    prevBalance,
                    newBalance
     ...
     *
     *
     * Note that [WalletAppKit.setAutoStop] is set by default and installs a shutdown handler
     * via [Runtime.addShutdownHook] so we do not need to worry about explicitly shutting down
     * the `WalletAppKit` if the process is terminated.
     */
    override fun close() {
        if (kit != null) {
            if (kit!!.isRunning) {
                kit!!.wallet()
                    .removeCoinsReceivedEventListener { wallet: Wallet, incomingTx: Transaction, prevBalance: Coin, newBalance: Coin ->
                        coinForwardingListener(
                            wallet,
                            incomingTx,
                            prevBalance,
    ...
    private fun coinForwardingListener(wallet: Wallet, incomingTx: Transaction, prevBalance: Coin, newBalance: Coin) {
        ...
        wallet.waitForConfirmations(incomingTx, REQUIRED_CONFIRMATIONS)
            .thenCompose { confidence: TransactionConfidence ->
                ...
                forward(wallet, incomingTx, forwardingAddress)
            }
            .whenComplete { broadcast: TransactionBroadcast?, throwable: Throwable ->
                if (broadcast != null) {
                    System.out.printf(
                        "Sent %s onwards and acknowledged by peers, via transaction %s\n",
                        broadcast.transaction().outputSum.toFriendlyString(),
    ...
    private fun forward(
        ...
        return wallet.sendTransaction(sendRequest)
            .thenCompose { broadcast: TransactionBroadcast ->
                System.out.printf(
                    "Transaction %s is signed and is being delivered to %s...\n",
                    broadcast.transaction().txId,
                    network
                )
    ...
    companion object {
        const val USAGE = "Usage: address-to-forward-to [mainnet|testnet|signet|regtest]"
        ...
        fun main(args: Array<String>) {
            ...
            ForwardingService(args).use { forwardingService ->
